...
def random_attack(G, source="N1", sink="N100", num_edges=5):
    ...
    edges_to_remove = random.sample(all_edges, min(num_edges, len(all_edges)))

    G.remove_edges_from(edges_to_remove)  # mutate global G for UI update
    flow_after, _ = nx.maximum_flow(G, source, sink, flow_func=nx.algorithms.flow.edmonds_karp)
    
    return flow_before, flow_after, edges_to_remove
...
def unified_callback(jump_source, jump_aim, reset, attack):
    ...
    if trigger == "jump-source-btn":
        ...
        attack_info = html.Div([
            html.H4("Random Attack Results"),
            html.P(f"Max Flow Before: {flow_before}"),
            html.P(f"Max Flow After: {flow_after}"),
            html.Strong("Removed Edges:"),
            html.Ul([html.Li(f"{u} → {v}") for u, v in removed_edges])
        ])
