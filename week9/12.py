#budget attack button is there but doesn't funnction
import dash
...
def budgeted_attack(G, source="N1", sink="N100", budget=50):
    ...
    flow_before, flow_dict = nx.maximum_flow(G_copy, source, sink, flow_func=nx.algorithms.flow.edmonds_karp)
    ...
    return flow_before, flow_after, affected_edges
...
app.layout = html.Div([
    ...
    html.Button("Run Budget Attack", id="attack-btn", n_clicks=0,
    ...
    html.Div(id="info-box", style={"position": "absolute", "top": "140px", "right": "20px", "width": "280px", "padding": "15px", "backgroundColor": "#E6E6FA", "border": "1px solid #999", "borderRadius": "10px", "boxShadow": "2px 2px 6px rgba(75, 0, 130)", "overflowY": "auto", "maxHeight": "80vh", "zIndex": 10}),
...
def unified_callback(jump_source, jump_aim, reset, attack):
    ...
    if trigger == "jump-source-btn":
        ...
        attack_info = html.Div([
            html.H4("Budget Attack Results"),
            html.P(f"Max Flow Before: {flow_before}"),
            html.P(f"Max Flow After: {flow_after}"),
            html.Strong("Attacked Edges:"),
            html.Ul([html.Li(f"{u} → {v} (Reduced by {reduction})") 
                    for u, v, reduction in affected_edges])
        ])
