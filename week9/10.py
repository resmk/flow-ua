...
def budgeted_attack(G, source="N1", sink="N100", budget=50):
    ...
    for u, v, cap in flow_edges:
        ...
        G[u][v]['capacity'] = max(1, cap - reduction)  # mutate the real graph
...
def unified_callback(jump_source, jump_aim, reset, attack):
    ...
    if trigger == "jump-source-btn":
        ...
        # Reset all edge colors and node positions
        ...
        for u, v, reduction in affected_edges:
            for link in graph_data['links']:
                if link["source"] == u and link["target"] == v:
                    link["color"] = "red"
                    link["capacity"] = G[u][v]["capacity"]
                    link["normCapacity"] = max(G[u][v]["capacity"] / max_capacity, 0.1)
        
        # Update JSON data
        global graph_data_json
        graph_data_json = json.dumps(graph_data)
        
        # Create attack results info
        ...
        # Return command to refresh graph and show info
        return "refresh", attack_info
