...
def random_attack(G, source="N1", sink="N100", num_edges=5):
    ...
    edges_to_remove = random.sample(list(G_copy.edges()), min(num_edges, len(G_copy.edges())))
     ...
     # Get capacities from the *copy* before deletion
    removed_edges_with_cap = [(u, v, G_copy[u][v]['capacity']) for u, v in edges_to_remove]

     # Now mutate G after extracting the capacities
...
def unified_callback(jump_source, jump_aim, reset, attack):
    ...
    # If no button has been pressed, do not update anything
    if not triggered:
    ...
    if trigger == "jump-source-btn":
        ...
        updated_data = create_graph_data(G)  # Assuming G is your graph object
        ...
        # For each removed edge, update its color in the graph data
        for u, v, _ in removed_edges:  # Added underscore for the unused capacity value
            for link in updated_data["links"]:
                if link["source"] == u and link["target"] == v:
                 link["color"] = "red"

        # Generate attack impact information
        attack_info = html.Div([
            html.H4("Random Attack Results", style={
                "color": "#8B0000", 
                "marginBottom": "10px",
            ...
            html.Div([
                html.Span("Max Flow Before:", style={"fontWeight": "bold", "width": "150px", "display": "inline-block"}),
                html.Span(f"{flow_before}", style={"color": "#006400", "float": "right"})
            ], style={"marginBottom": "5px"}),
            html.Div([
                html.Span("Max Flow After:", style={"fontWeight": "bold", "width": "150px", "display": "inline-block"}),
                html.Span(f"{flow_after}", style={"color": "#8B0000", "float": "right"})
            ], style={"marginBottom": "5px"}),
            html.Div([
                html.Span("Flow Reduction:", style={"fontWeight": "bold", "width": "150px", "display": "inline-block"}),
                html.Span(f"{((flow_before - flow_after) / flow_before * 100):.1f}%", 
                          style={"color": "#8B0000" if flow_after < flow_before else "#006400", "float": "right"})
            ], style={"marginBottom": "10px"}),
            html.Div([
                html.Span("Total Capacity Removed:", style={"fontWeight": "bold", "width": "150px", "display": "inline-block"}),
                html.Span(f"{sum(cap for _, _, cap in removed_edges)}", style={"color": "#8B0000", "float": "right"})
            ])
        ], style={
...
app.clientside_callback(
    ...
    function(action,updatedGraphData) {{
        const graphData = JSON.parse(updatedGraphData || '{graph_data_json}');
        ...
        if (!window.fgInstance) {{
            const interval = setInterval(() => {{
                ...
                fg.graphData(graphData)
                    ...
                    .onNodeClick(node => {{
                        // Reset all node colors except special nodes
                        ...
                        // Set clicked node to red
                        node.color = 'red';
                        
                        // Update the node's three object
