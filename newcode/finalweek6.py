#🔴 Red if the node only has outgoing edges 
#🔵 Blue if the node has both incoming and outgoing
...
# Guarantee path from N1 to N1000
...
app.layout = html.Div([
    ...
    html.Button("Color Nodes by Type", id="color-nodes-btn", n_clicks=0,
                title="Red if the node only has outgoing edges\n Blue if the node has both incoming and outgoing",
...
@app.callback(
    ...
    [Input("jump-source-btn", "n_clicks"),
     ...
     Input("color-nodes-btn", "n_clicks")]
)
def determine_action(source_clicks, aim_clicks, reset_clicks, color_clicks):
...
app.clientside_callback(
    ...
    function(action) {{
        ...
        if (window.fgInstance) {{
            ...
            }} else if (action === "color-nodes") {{
                const nodes = window.fgInstance.graphData().nodes;
                for (const node of nodes) {{
                    if (node.isRoot || node.isAim) continue;
                    const incoming = nodeInfo[node.id].incoming.length;
                    const outgoing = nodeInfo[node.id].outgoing.length;
