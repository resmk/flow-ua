...
extra_edges = []
for _ in range(200):
    u = f"N{random.randint(1, 199)}"
    v = f"N{random.randint(int(u[1:]) + 1, 200)}"
    if u != v and u != "N100":
...
info_box_style = {**box_style, "top": "140px", "right": "20px", "width": "300px"}
affected_edges_style = {**box_style, "top": "260px", "left": "20px", "width": "250px", "backgroundColor": "#FFF8F8", "border": "1px solid #FFCCCC"}
capacity_box_style = {**box_style, "top": "500px", "right": "20px", "width": "300px", "backgroundColor": "#F0F8FF", "border": "1px solid #ADD8E6"}

sidebar_style = {
    "position": "absolute",
    "top": "20px",
    "left": "20px",     # or "right": "20px" if you want it on the right
    "width": "320px",
    "maxHeight": "90vh",
    "overflowY": "auto",
    "display": "flex",
...
def find_paths_to_source_and_aim(G, u, v):
    """Find paths from source (N1) to u and from v to aim (N100)"""
    try:
...
def unified_callback(jump_source, jump_aim, reset, attack, show_attack, back_main, 
                    current_graph_data, attacked_edges_data, original_graph_data,
    ...
    if trigger == "jump-source-btn":
        return "jump-N1", dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update
    elif trigger == "jump-aim-btn":
        return "jump-N100", dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update
    elif trigger == "reset-btn":
        original_data = json.loads(original_graph_data)
        global G
        G = nx.DiGraph()
        G.add_nodes_from([f"N{i}" for i in range(1, 101)])
        for u, v, cap in edges:
            if u != "N100":
                G.add_edge(u, v, capacity=cap)
        return ("reset", dash.no_update, original_graph_data, dash.no_update, {"display": "none"}, dash.no_update, {"display": "none"}, [], original_graph_data, original_graph_data)
    
    elif trigger == "attack-btn":
        # Store the current state before attack
        ...
        return ("refresh", attack_info, json.dumps(updated_data), 
                ...
                {"display": "block", "position": "absolute", "top": "260px", "left": "20px", 
                ...
                capacity_content,
                {"display": "block", "position": "absolute", "top": "calc(230px + 320px)", "left": "20px", 
        ...
        # Get the current full graph data
        current_data = json.loads(current_graph_data)
        
        # Create a filtered version showing attacked edges, their nodes, and paths
        attacked_edges = [(e["source"], e["target"]) for e in attacked_edges_data]
        nodes_to_show = set()
        links_to_show = []
        
        # Find all nodes and links in the paths
        for u, v in attacked_edges:
            # Add the attacked edge itself
            nodes_to_show.add(u)
            nodes_to_show.add(v)
            
            # Find paths from source to u and from v to aim
            path_from_source, path_to_aim = find_paths_to_source_and_aim(G, u, v)
            
            # Add nodes and links from source path
            ...
            # Add nodes and links to aim path
        ...
        # Add all relevant links (attacked edges and path edges)
        all_links_to_show = attacked_edges + list(set(links_to_show))
        
        # Find the link data from the original graph data
        for link in current_data["links"]:
            if (link["source"], link["target"]) in all_links_to_show:
                new_link = link.copy()
                # Color attacked edges red, path edges blue
        ...
        return "show-attacked", dash.no_update, json.dumps(filtered_data), dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update
    
    elif trigger == "back-main-btn":
        return "back-to-main", dash.no_update, pre_attack_graph_data, dash.no_update, {"display": "none"}, dash.no_update, {"display": "none"}, dash.no_update, dash.no_update, dash.no_update
