...
@app.callback(
    ...
    Input("restore-btn", "n_clicks"),  # New input
...
def unified_callback(jump_source, jump_aim, reset, restore, budgeted_attack_clicks, show_attack, multi_step_attack_click,
    ...
    if trigger == "restore-btn":
        # Restore the graph to its pre-attack state
        if pre_attack_graph_data:
            return (
                "refresh",
                html.Div([html.H4("Graph Restored"), html.P("Restored to state before last attack")]),
    ...
    if trigger == "jump-source-btn":
        ...
        global G
        G = copy.deepcopy(ORIGINAL_GRAPH)  # Restore the complete original graph
        
        # Create fresh graph data from the restored complete graph
        updated_data = create_graph_data(G)
        
        return (
            "reset",
            dash.no_update,  # info-box children
            json.dumps(updated_data),  # graph-data-store
            dash.no_update,  # affected-edges-box children
            {"display": "none"},  # affected-edges-box style
            dash.no_update,  # capacity-box children
            {"display": "none"},  # capacity-box style
            [],  # attacked-edges-store (clear it)
            json.dumps(updated_data),  # original-graph-store
            json.dumps(updated_data),  # pre-attack-graph-store
        )
