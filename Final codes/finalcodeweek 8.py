...
graph_data = {
    "nodes": [{"id": node, "name": node, "isRoot": node == root_node, "isAim": node == "N100"} for node in G.nodes()],
    "links": [{
        "source": u, "target": v,
...
app = DashProxy(__name__, external_scripts=external_scripts, transforms=[TriggerTransform(), MultiplexerTransform()])
server = app.server
...
app.layout = html.Div([
    html.Div([
        ...
        html.Button("Run Shortest Path", id="shortest-path-btn", n_clicks=0,
              ...
              "boxShadow": "2px 2px 6px rgba(0,0,0,0.3)", "minWidth": "200px"}),

    html.Button("Reset Node", id="reset-btn", n_clicks=0,
                style={**button_style, "position": "absolute", "top": "20px", "left": "20px", "zIndex": 11}),
    html.Button("Jump to Source (N1)", id="jump-source-btn", n_clicks=0,
                style={**button_style, "position": "absolute", "top": "80px", "left": "20px", "zIndex": 11}),
    html.Button("Jump to Aim (N100)", id="jump-aim-btn", n_clicks=0,
                style={**button_style, "position": "absolute", "top": "140px", "left": "20px", "zIndex": 11}),
...
def unified_callback(jump_source, jump_aim, reset, path_btn, source, target):
    ...
    if trigger == "jump-source-btn":
        ...
        try:
            # Use capacity as weight: smaller 1/cap means stronger path
            ...
            for u, v in path_edges:
                ...
                cost = 1 / cap
                total_cost += cost
                edge_details += f"<li>{u} → {v} — Capacity: {cap}, Cost: {cost:.4f}</li>"
            ...
            info_html = f"""
                <h4>Shortest Path from {source} to {target} (Capacity-Based)</h4>
                <p>Nodes in path: {len(path)}</p>
                <p>Total path cost (\u2211 1/capacity): {total_cost:.4f}</p>
                <ul>{edge_details}</ul>
            """

            return json.dumps(js_command), info_html
