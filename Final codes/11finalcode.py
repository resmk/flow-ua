...
def unified_callback(jump_source, jump_aim, reset, attack, show_attack, back_main, 
    ...
    if trigger == "jump-source-btn":
        ...
        # Create a filtered version showing only attacked edges and their nodes
        attacked_edges = [(e["source"], e["target"]) for e in attacked_edges_data]
        attacked_nodes = set()
        
        # Find all nodes involved in attacked edges
        ...
        filtered_data = {
            "nodes": [node for node in current_data["nodes"] if node["id"] in attacked_nodes],
            "links": [link for link in current_data["links"] 
                     if (link["source"], link["target"]) in attacked_edges]
        }
        
        # Ensure all links are colored red
        ...
        # Return to the state right before the attack was performed
