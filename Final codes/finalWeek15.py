#adjacency list → 3D graph (Dash + ForceGraph3D)
...
def budgeted_attack(G, source, sink, budget=300):
    """
    Simulates an attack on the network by reducing capacities of edges in the max-flow path.
    - G: NetworkX graph (modified in place)
    - source: source node ID (string)
    - sink: target node ID (string)
    - budget: total allowed reduction in capacity across edges

    Returns:
        flow_value_before: Max flow before attack
        flow_value_after: Max flow after attack
        affected_edges: List of (u, v, reduction_amount) for modified edges
    """

    # Copy the graph to calculate initial flow without modifying the original
    G_copy = copy.deepcopy(G)

    # Compute max flow before attack
    flow_value_before, flow_dict = nx.maximum_flow(
        G_copy, source, sink,
        flow_func=nx.algorithms.flow.edmonds_karp
    )

    # Find edges that actually carry flow
    flow_edges = [
        (u, v, G_copy[u][v]['capacity'])
        for u in flow_dict for v in flow_dict[u]
        if flow_dict[u][v] > 0
    ]

    # Sort edges by capacity in descending order (attack largest capacity edges first)
    flow_edges.sort(key=lambda x: x[2], reverse=True)

    total_reduced = 0  # Total capacity reduction used
    affected_edges = []  # Store affected edges and reduction amounts

    # Reduce capacities until budget is used up
    for u, v, cap in flow_edges:
        if total_reduced >= budget:
            break  # Stop if budget exhausted

        # Amount to reduce: not below 1, not exceeding remaining budget
        reduction = min(cap - 1, budget - total_reduced)

        # Apply reduction to the original graph
        G[u][v]['capacity'] = max(1, cap - reduction)

        total_reduced += reduction
        affected_edges.append((u, v, reduction))

    # Compute max flow after attack
...
def create_color_legend():
    return html.Div([
        ...
        html.Div([
            html.Span("⬤", style={"color": "green", "marginRight": "6px"}),
            html.Span(f"Source Node ({source_node})", style={"fontSize": "14px"})
        ], style={"marginBottom": "6px"}),
        html.Div([
            html.Span("⬤", style={"color": "deeppink", "marginRight": "6px"}),
            html.Span(f"Target Node ({target_node})", style={"fontSize": "14px"})
        ])
    ], style={
...
def find_paths_to_source_and_aim(G, u, v):
    try:
        path_from_source = nx.shortest_path(G, source_node, u)
    except nx.NetworkXNoPath:
        path_from_source = []
    try:
        path_to_aim = nx.shortest_path(G, v, target_node)
    except nx.NetworkXNoPath:
        path_to_aim = []
...
app.layout = html.Div([
    html.Div([
        ...
        html.Button(f"Jump to Source ({source_node})", id="jump-source-btn", n_clicks=0, style=button_style),
        html.Button(f"Jump to Aim ({target_node})", id="jump-aim-btn", n_clicks=0, style=button_style),
...
def unified_callback(jump_source, jump_aim, restore, budgeted_attack_clicks, show_attack, multi_step_attack_click,
    ...
    if trigger == "restore-btn":
        ...
        return "jump-" + source_node, *([dash.no_update] * 9)

    elif trigger == "jump-aim-btn":
        return "jump-" + target_node, *([dash.no_update] * 9)

    elif trigger == "budgeted-attack-btn":
        pre_attack_data = current_graph_data
        total_capacity_before = calculate_total_capacity(G)
        flow_before, flow_after, affected_edges = budgeted_attack(G, source_node, target_node, budget=300)
        ...
        for link in updated_data["links"]:
            if (link["source"], link["target"]) in attacked_edge_pairs:
                link["color"] = "red"  # attacked edges to red

        total_capacity_after = calculate_total_capacity(G)
        severity = (flow_before - flow_after) / flow_before if flow_before > 0 else 0
        ...
        capacity_content = html.Div([
            ...
            html.P(f"Percentage: {((total_capacity_before - total_capacity_after) / total_capacity_before * 100):.1f}%" if total_capacity_before > 0 else "N/A"),
        ])
        ...
        pre_attack_data = current_graph_data
        flow_before, flow_after, affected_edges = multi_step_attack(G, source_node, target_node, steps=3, edges_per_step=10)
        ...
        capacity_content = html.Div([
            ...
            html.P(f"Percentage: {((flow_before - flow_after) / flow_before * 100):.1f}%" if flow_before > 0 else "N/A"),
        ])
        ...
        try:
            all_paths = list(nx.all_simple_paths(G, source=source_node, target=target_node, cutoff=20))
        except nx.NetworkXNoPath:
            return dash.no_update, *([dash.no_update] * 9)
...
# Clientside renderer (ForceGraph3D) with dynamic source/target colors
app.clientside_callback(
    """
